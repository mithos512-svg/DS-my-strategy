...
status: bullet points ready — redesigned 09.08 to start from systems-thinking basics
created: 2026-08-09
language: en (team is English-speaking)
---

# Systems Thinking Workshop, Session 1 — Bullet Points (EN, redesigned for a non-technical audience)

> Redesigned 09.08: the original version jumped straight into FPF vocabulary
> ("holon," "meta-holon transition," "MHTTriggerProfile"). For a team without
> systems-thinking background, that's jargon with no foothold. This version
> starts from "what is a system" and builds up to the same practical
> 4-question check — using only plain language in the actual talking points.
> FPF pattern names kept in the facilitator note at the end, not spoken aloud.
>
> Renamed 09.08 from "Book Club" to "Systems Thinking Workshop" per F.18
> naming decision — see namecard-systems-thinking-workshop.md.
...
## Block 3 — CORE: the 4-question check (20 min)

If a quick fix to the measurement doesn't resolve it, walk through these four
...
|---|---|
| **Who's included?** | Did one unit widen who/what counts (e.g. added contractors) while another didn't? |
| **What's it made of?** | Does the number include the same components? Tax in or out; returns in or out? |
| **What does it depend on?** | Does one unit's number rely on a process or step that the other unit simply doesn't have? |
| **Is it steady or does it jump?** | Stable in one unit, swings in another — because they're counted over different time windows? |

- Run each metric from the warm-up through all four questions. Find at least
...
- The 4-question check applies to any disputed metric before it turns into a
  conflict.
...
"Meta-Holon Transition" (the 4 questions = Boundary/Composition/Coordination/
Stability from MHTTriggerProfile — FPF Seminar 2, part 1, slide 26) and the
optional block maps to F.9 "Bridge and Alignment across Contexts" (FPF Seminar
4, part 1, slide 11). Systems-thinking foundation (emergence, nesting,
meta-system transition, car example) — course "Введение в системное мышление,"
§2.01 and §5.5. Keep FPF names and jargon out of the actual talking points;
the room doesn't need the labels to get the value.
